...
import { resolveEpisodeAliases } from './resolve-aliases.ts'
import type { AliasRecord, EpisodeDecision, ParsedEpisode, PublishedScore } from './score-types.ts'
...
export async function decideEpisodeImport(
  ...
  if (published.length > 0 && resolved.scores.length < Math.ceil(published.length * 0.5)) {
    return {
      status: 'needs_review',
      episodeNumber: episode.episodeNumber,
      code: 'payload_shrink',
